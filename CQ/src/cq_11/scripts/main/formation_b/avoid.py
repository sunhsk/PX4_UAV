...
def avoid(vehicle_type_b,vehicle_num_b,control_type_b='vel'):
    # rospy.init_node('avoid')vehicle_type = sys.argv[1]
    global vehicle_type, vehicle_num,control_type,pose,pose_sub,avoid_control_pub,avoid_radius,aid_vec1,aid_vec2,vehicles_avoid_control
    vehicle_type = vehicle_type_b
    vehicle_num = vehicle_num_b
    control_type = control_type_b
    pose = [None]*vehicle_num
    pose_sub = [None]*vehicle_num
    avoid_control_pub = [None]*vehicle_num
    avoid_radius = 1.5
    ...
    for i in range(vehicle_num):
        ...
        pose_sub[i] = rospy.Subscriber(vehicle_type+'_'+str(i)+'/mavros/vision_pose/pose',PoseStamped,pose_callback,i,queue_size=1)
        if control_type == "vel":
