...
{
    ros::init(argc, argv, "pose");         // node name
    ...
    ros::Publisher attitude_pub = nh.advertise<mavros_msgs::AttitudeTarget>("/iris_0/mavros/setpoint_raw/attitude",10);
    ...
    ros::ServiceClient arming_client = nh.serviceClient<mavros_msgs::CommandBool>("iris_0/mavros/cmd/arming");//向 PX4 发送解锁（arm）或上锁（disarm）请求
    ros::ServiceClient set_mode_client = nh.serviceClient<mavros_msgs::SetMode>("iris_0/mavros/set_mode");//	向 PX4 请求切换飞行模式（如 OFFBOARD、AUTO.TAKEOFF 等）
    ...
    while(ros::ok()){
        ...
        if( current_state.mode != "OFFBOARD" &&
            ...
            if( set_mode_client.call(offb_set_mode) &&      //set_mode_client.call(offb_set_mode)向指定的服务端发送一个请求，并等待服务端返回响应结果。
                ...
                ROS_INFO("Offboard/home/hsk/PX4_Firmware/launch/CQ/multi_vehicle_3.launch enabled");
            }
