...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>  //mavros_msgs::State：属于 mavros_msgs 包中的 State 消息类型，用于表示 MAVROS 无人机的状态信息。
            ("mavros/state", 10, state_cb);                       //订阅 mavros/state 话题，用于获取无人机的状态信息。（如飞行模式、是否解锁等）。 在 PX4 + MAVROS 系统中，/mavros/state 话题用于发布无人机的状态，包括： 当前飞行模式（如 OFFBOARD、AUTO.LOITER）;是否解锁（马达是否启动）等
    ...
    ros::Subscriber get_point = nh.subscribe<geometry_msgs::PoseStamped>   //订阅 mavros/local_position/pose 话题，用于获取无人机的位置信息。PoseStamped 是 ROS 中的一个标准消息类型，表示一个带有时间戳的三维位姿（位置 + 姿态）。
            ("mavros/local_position/pose", 10, getpointfdb);                //"mavros/local_position/pose"，MAVROS 提供的一个 本地位置 话题。
            
    ros::Publisher local_pos_pub = nh.advertise<geometry_msgs::PoseStamped>
            ("mavros/setpoint_position/local", 10);                         //"mavros/setpoint_position/local"，这是 MAVROS 话题，用于接收无人机的本地目标位置指令。当你想让无人机飞到一个指定的目标位置时，可以向该话题发布 PoseStamped 消息。
    ...
    ros::ServiceClient arming_client = nh.serviceClient<mavros_msgs::CommandBool>("mavros/cmd/arming");//向 PX4 发送解锁（arm）或上锁（disarm）请求
    ros::ServiceClient set_mode_client = nh.serviceClient<mavros_msgs::SetMode>("mavros/set_mode");//	向 PX4 请求切换飞行模式（如 OFFBOARD、AUTO.TAKEOFF 等）
